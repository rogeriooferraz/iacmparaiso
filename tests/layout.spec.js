...
const eventsConfig = JSON.parse(
  fs.readFileSync(path.join(__dirname, '..', 'events', 'events.json'), 'utf8')
);
...
async function checkLayout(device, width, height) {
  ...
  try {
    ...
    // IMPORTANT: Wait for the CSS fade-in translation animation to finish.
    // The `.reveal` class uses `transform: translateY(24px)` -> `0` over 0.7s.
    // Measuring bounding boxes before it finishes causes false positive overflow errors.
    ...
    // Test 1: Buttons must be visible without scrolling
    ...
    // Test 2: Outer beige margins must be perfectly symmetric everywhere
    const mediaBox = await (await page.$('.hero-media')).boundingBox();
    // Since we wait for animation, y should be exactly the height of the topbar + margin
    ...
    // Test 3: Inner white margins must dynamically adapt via object-fit
    ...
    // 1) All 4 margins exist
    // 2) The narrowest margin size == 8px
    // 3) Right margin == Left margin
    // 4) Top margin == Bottom margin
    
    // In our calculation, whiteMarginTop applies to both top and bottom symmetrically, 
    // and whiteMarginLeft applies to both left and right symmetrically due to object-fit: contain.
    const margins = {
    ...
    // Test 4: Eventos navigation target and heading must exist without changing the anchor
    const navHref = await page.$eval('.hero-nav a[href="#eventos"]', (link) => link.getAttribute('href'));
    ...
    console.log(`- Eventos nav href: ${navHref}, headings: ${eventStructure.headingTexts.join(' | ')}`);
    console.log(`- Heading alignment -> sobre: ${eventStructure.aboutHeadingLeft}px, eventos: ${eventStructure.headingLeft}px, local: ${eventStructure.localHeadingLeft}px, contato: ${eventStructure.contactHeadingLeft}px`);
    ...
    if (JSON.stringify(eventStructure.headingTexts) !== JSON.stringify(expectedTitles)) {
      console.error(`  ERROR: Event headings do not match events.json order on ${device}!`);
      process.exitCode = 1;
    }

    if (JSON.stringify(eventStructure.imageSources) !== JSON.stringify(expectedFiles)) {
      console.error(`  ERROR: Event images do not match events.json order on ${device}!`);
      process.exitCode = 1;
    }

    if (!eventStructure.firstHeadingBeforeFrame) {
      console.error(`  ERROR: First event heading is not positioned before the first image on ${device}!`);
      process.exitCode = 1;
    }

    if (Math.abs(eventStructure.headingLeft - eventStructure.localHeadingLeft) > 1) {
      console.error(`  ERROR: Eventos heading is not aligned with the Local section heading on ${device}!`);
      process.exitCode = 1;
    }

    if (Math.abs(eventStructure.aboutHeadingLeft - eventStructure.localHeadingLeft) > 1) {
      console.error(`  ERROR: Sobre heading is not aligned with the Local section heading on ${device}!`);
      process.exitCode = 1;
    }

    if (eventStructure.contactHeadingText !== 'Igreja Aliança do Paraíso') {
      console.error(`  ERROR: Contato heading text is incorrect on ${device}!`);
      process.exitCode = 1;
    }

    if (Math.abs(eventStructure.contactHeadingLeft - eventStructure.localHeadingLeft) > 1) {
      console.error(`  ERROR: Contato heading is not aligned with the Local section heading on ${device}!`);
      process.exitCode = 1;
    }

    // Test 5: Event image must use an 8px grid frame and keep viewport-height sizing
    ...
    if (isSmallScreen) {
      ...
      if (anchorMetrics.entryBottom > anchorMetrics.viewportHeight + 2) {
        console.error(`  ERROR: First event entry is taller than the viewport on large screens on ${device}!`);
        process.exitCode = 1;
      }

      if (anchorMetrics.imageBottom > anchorMetrics.viewportHeight - 6) {
        console.error(`  ERROR: Event image is missing visible bottom white margin on large screens on ${device}!`);
        process.exitCode = 1;
      }
    }
    ...
    const eventMetrics = await page.evaluate(() => {
      ...
      return {
        ...
        headingHeight: Math.round(document.querySelector('.events-heading-shell').getBoundingClientRect().height),
    ...
    if (isSmallScreen) {
      if (eventMetrics.imageHeight >= (eventMetrics.viewportHeight - 16)) {
...
(async () => {
  ...
  // Test Pixel 7 (Taller screen)
  await checkLayout('pixel7', 412, 915);
  
  // Test Pixel 5 (Medium screen)
  await checkLayout('pixel5', 393, 851);
  
  // Test Moto G4 (Short screen)
